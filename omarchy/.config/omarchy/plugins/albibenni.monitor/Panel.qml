...
Panel {
  ...
  property int pendingScrollSteps: 0
  readonly property string pluginDir: Quickshell.env("HOME") + "/.config/omarchy/plugins/albibenni.monitor"

  // Carry sub-notch touchpad deltas between wheel events.
  property real wheelAccumulator: 0

  // A bar is instantiated once per output. Use its owning screen for pointer
  // actions so scrolling a Display icon always controls that display, even
  // while keyboard focus remains on another monitor.
  function barMonitorName() {
    // QsWindow, rather than QtQuick.Window, is the attachment supplied by the
    // Omarchy bar's PanelWindow. This mirrors Bar.qml's `targetWindow` helper.
    // The bar queries this attachment on the loaded widget root (`activeItem`),
    // not on a nested button. The root is guaranteed to belong to the bar's
    // PanelWindow on every output.
  ...
  function brightnessMonitor() {
    return root.barMonitorName() || root.focusedMonitor
  }
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["omarchy-brightness-display", "--no-osd", "--monitor", root.brightnessMonitor(), percent + "%"]
  ...
  // Wheel events happen over a specific bar output, independently of the
  // popup's focused-monitor state. Resolve the output under the pointer in a
  // helper and use a relative step so it reads that display's own brightness.
  function scrollBrightness(steps) {
    root.pendingScrollSteps += steps
    if (!scrollBrightnessProc.running) root.runPendingScrollBrightness()
  }

  function runPendingScrollBrightness() {
    if (root.pendingScrollSteps === 0) return
    var steps = root.pendingScrollSteps
    root.pendingScrollSteps = 0
    scrollBrightnessProc.command = ["bash", root.pluginDir + "/brightness-at-cursor.sh", String(steps * 5)]
    scrollBrightnessProc.running = true
  }
  ...
  // The stock monitor-state command reports the focused output. Query this
  // bar's output separately, so the scroll delta starts from its own current
  // brightness instead of the primary display's value.
  function refreshBarBrightness() {
    var monitor = root.barMonitorName()
    if (!monitor || brightnessProc.running) return
    brightnessProc.command = ["omarchy-brightness-display", "--monitor", monitor]
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var brightness = String(text || "").trim()
        root.brightnessAvailable = brightness !== ""
        if (root.brightnessAvailable)
          root.brightnessPercent = Model.clampBrightness(brightness)
      }
    }
  }
  ...
  Process {
    ...
    onRunningChanged: if (!running && root.pendingScrollSteps !== 0)
      root.runPendingScrollBrightness()
  }
  ...
  BarIconButton {
    ...
    onWheelMoved: function(delta) {
      ...
      root.scrollBrightness(wheel.steps)
    }
  }
